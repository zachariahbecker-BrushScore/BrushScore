...
const AWARD_OPTIONS = [
  ...
  { value: 'best_category', label: "Judge's Choice" },
  { value: 'best_show', label: 'Best in Show' },
];

const DEFAULT_CATEGORIES = [
  { name: 'Armor & Military Vehicles', classes: ['1/72', '1/48', '1/35'] },
  { name: 'Aircraft', classes: ['1/72', '1/48', '1/32'] },
  { name: 'Automotive', classes: ['Stock', 'Custom'] },
  { name: 'Sci-Fi & Fantasy', classes: ['Any Scale'] },
  { name: 'Figures & Busts', classes: ['54mm & Under', '1/16 & Larger'] },
  { name: 'Ships & Watercraft', classes: ['Any Scale'] },
  { name: 'Junior (Under 18)', classes: ['Any Category'] },
];
...
function awardRank(v) { const order = ['best_category', 'first', 'second', 'third', 'best_class', 'honorable']; const i = order.indexOf(v); return i === -1 ? 99 : i; }
...
function entryQrPayload(number) { return `SHOWBENCH-ENTRY-${number}`; }
function parseEntryQr(text) {
  const m = /^SHOWBENCH-ENTRY-(\d+)$/.exec((text || '').trim());
